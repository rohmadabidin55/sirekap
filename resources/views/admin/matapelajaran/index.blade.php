...
@section('content')
<div x-data="mapelCrud()">
    <div class="flex flex-col sm:flex-row justify-between items-center">
        <h3 class="text-gray-700 text-3xl font-medium">Data Mata Pelajaran</h3>
    ...
    <div class="mt-6 bg-white p-4 rounded-md shadow-sm border border-gray-200">
        <form x-data action="{{ route('admin.matapelajaran.index') }}" method="GET">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div class="lg:col-span-2">
                    <label for="search" class="text-sm">Cari Kode atau Nama Mapel</label>
        ...
        <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
                <table class="min-w-full">
                    <thead>
                        <tr>
                            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Kode Mapel</th>
                            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Nama Mata Pelajaran</th>
                    ...
                    <tbody class="bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                    <div class="flex justify-center items-center space-x-4">
                                        <button @click="edit({{ $mapel }})" class="text-gray-500 hover:text-indigo-600" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                                        </button>
                                        <button @click="openDeleteModal({{ $mapel->id }})" class="text-gray-500 hover:text-red-600" title="Hapus">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500">Tidak ada data yang cocok.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all">
            <form @submit.prevent="saveMapel()">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900" x-text="isEditMode ? 'Edit Mata Pelajaran' : 'Tambah Mata Pelajaran'"></h3>
                    <div class="mt-4 space-y-4">
                        <div>
                            <label for="kode_mapel" class="block text-sm font-medium text-gray-700">Kode Mapel</label>
                            <input type="text" x-model="formData.kode_mapel" id="kode_mapel" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required placeholder="Contoh: MTK-01">
                            <span x-show="errors.kode_mapel" x-text="errors.kode_mapel ? errors.kode_mapel[0] : ''" class="text-red-500 text-xs mt-1"></span>
                        </div>
                        <div>
                            <label for="nama_mapel" class="block text-sm font-medium text-gray-700">Nama Mata Pelajaran</label>
                            <input type="text" x-model="formData.nama_mapel" id="nama_mapel" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required placeholder="Contoh: Matematika Wajib">
                            <span x-show="errors.nama_mapel" x-text="errors.nama_mapel ? errors.nama_mapel[0] : ''" class="text-red-500 text-xs mt-1"></span>
                        </div>
                    </div>
                </div>
    ...
    <div x-show="isDeleteModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all">
            <div class="p-6 text-center">
                ...
                <h3 class="mt-2 text-lg font-medium text-gray-900">Hapus Data Mata Pelajaran</h3>
                <p class="mt-2 text-sm text-gray-500">Apakah Anda yakin ingin menghapus data ini?</p>
            </div>
            <div class="bg-gray-50 px-6 py-4 flex justify-center space-x-3">
                ...
                <button @click="deleteMapel()" type="button" class="inline-flex justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-red-700">Ya, Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    function mapelCrud() {
        return {
            ...
            resetForm() {
                this.formData = { id: null, kode_mapel: '', nama_mapel: '' };
                this.isEditMode = false;
                this.errors = {};
            },

            edit(mapel) {
                this.formData = { id: mapel.id, kode_mapel: mapel.kode_mapel, nama_mapel: mapel.nama_mapel };
            ...
            async saveMapel() {
                this.errors = {};
                let url = this.isEditMode ? `/admin/matapelajaran/${this.formData.id}` : '{{ route("admin.matapelajaran.store") }}';
            ...
            openDeleteModal(id) { this.mapelIdToDelete = id; this.isDeleteModalOpen = true; },
            closeDeleteModal() { this.isDeleteModalOpen = false; this.mapelIdToDelete = null; },

            async deleteMapel() {
                if (!this.mapelIdToDelete) return;
                const response = await fetch(`/admin/matapelajaran/${this.mapelIdToDelete}`, {
                    method: 'DELETE',
